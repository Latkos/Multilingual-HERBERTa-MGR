from utils.enhancement import (
    enhance_with_nothing,
    enhance_with_entity,
    enhance_with_brackets,
    enhance_with_entity_differentiated,
    enhance_with_special_characters,
    enhance_entities_only,
)
from utils.prediction import train_re_on_ner, save_results


def test_enhancing_text_used_to_train_re(
    train_df, test_df, ner_model, re_model, results_file="results_optimized_ner.txt", read=True, train_ner=False
):
    enhancements = [
    ...
    for enhancement_name, enhancement_func in enhancements:
        result = train_re_on_ner(
            ...
            results_file,
            read,
            model_path=f"models/re_{enhancement_name}",
        )
        f1 = result["f1"]
        f1_values.append(f1)
