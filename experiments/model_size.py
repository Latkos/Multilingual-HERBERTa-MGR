...
def test_ner_quality_depending_on_dataset_size(model, train_df, test_df, sizes, random_state=None):
    results = []
    for size in sizes:
...
def test_re_quality_depending_on_dataset_size(model, train_df, test_df, sizes, random_state=None, remove_tags=False):
    results = []
    for size in sizes:
